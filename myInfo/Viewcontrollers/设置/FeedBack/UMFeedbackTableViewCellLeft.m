...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        _timestampLabel.frame = CGRectMake(0.0f, 4, self.bounds.size.width, 18);

        [self.contentView addSubview:_timestampLabel];

        _messageBackgroundView = [[UIImageView alloc] initWithFrame:self.textLabel.frame];
        _messageBackgroundView.image = [[UIImage imageNamed:@"messages_left_bubble.png"] stretchableImageWithLeftCapWidth:20 topCapHeight:16];
...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextMoveToPoint(context, 0, 14); //start at this point
    CGContextAddLineToPoint(context, (self.bounds.size.width - 130) / 2, 14); //draw to this point

    CGContextMoveToPoint(context, self.bounds.size.width, 14); //start at this point
    CGContextAddLineToPoint(context, self.bounds.size.width - (self.bounds.size.width - 136) / 2, 14); //draw to this point
...
- (void)dealloc
{
    ...
    [_messageBackgroundView release];
    [super dealloc];
}
