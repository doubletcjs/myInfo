...
- (void)makeToastActivityWithMsg:(NSString *)msg
{
    [self makeToastActivity:CSToastActivityDefaultPosition withMsg:msg];
}
- (void)makeToastActivity:(id)position withMsg:(NSString *)msg
{
    // sanity
    UIView *existingActivityView = (UIView *)objc_getAssociatedObject(self, &CSToastActivityViewKey);
    if (existingActivityView != nil) return;
    
    UIView *activityView = [[[UIView alloc] initWithFrame:CGRectMake(0, 0, 120, 120)] autorelease];
    ...
    activityIndicatorView.center = CGPointMake(activityView.bounds.size.width / 2, (activityView.bounds.size.height-10) / 2);
    [activityView addSubview:activityIndicatorView];
    [activityIndicatorView startAnimating];
    
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, activityView.frame.size.height-24, activityView.frame.size.width, 20)];
...
- (void)makeToastActivityDuration:(CGFloat)interval withMsg:(NSString *)msg
{
    [self makeToastActivityWithMsg:msg];
    [self performSelector:@selector(hideToastActivity) withObject:self afterDelay:interval];
}

- (void)makeToastActivity:(id)position duration:(CGFloat)interval withMsg:(NSString *)msg
{
    [self makeToastActivity:position withMsg:msg];
    [self performSelector:@selector(hideToastActivity) withObject:self afterDelay:interval];
}
...
- (void)makeToastActivityDuration:(CGFloat)interval
{
    [self makeToastActivity];
    [self performSelector:@selector(hideToastActivity) withObject:self afterDelay:interval];
}

- (void)makeToastActivity:(id)position duration:(CGFloat)interval
{
